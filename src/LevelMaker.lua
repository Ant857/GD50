...
function LevelMaker.generate(width, height)
    ...
    while lockLocation > keyLocation do
        lockLocation = math.random(width - 4)
    end
    ...
    for x = 1, width do
        ...
        if math.random(7) == 1 and (x < width - 3) and (x ~= keyLocation) and (x ~= lockLocation) then
            for y = 7, height do
                table.insert(tiles[y],
                    Tile(x, y, tileID, nil, tileset, topperset))
            end
                                ...
                                if hasKey then
                                    ...
                                    local pole = GameObject {
                                            ...
                                            onConsume = function(player, object)
                                                ...
                                                gSounds['pickup']:play()
                                                gStateMachine:change('play', {previousWidth = width})
                                                print(width)
                                            end
                                    }
