...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        if love.keyboard.wasPressed('space') and not hit and not tripled then
          
            --set to true to show they're are now three aliens to render
            tripled = true
            
            --spawn two new aliens and set their velocities so one goes above and one goes below
            ...
            self.alien2.body:setLinearVelocity((xVel), (yVel+50))
            self.alien3.body:setLinearVelocity((xVel), (yVel-50))
...
function AlienLaunchMarker:render()
    if not self.launched then
        ...
        --render all three aliens
        self.alien:render()
        if self.alien2 ~= nil then
            if self.alien2.destroyed == false then
                self.alien2:render()
            end
        end
        if self.alien3 ~= nil then
            if self.alien3.destroyed == false then
                self.alien3:render()
            end
        end
    end
