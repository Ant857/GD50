...
function PlayState:init()
    ...
    self.noMoreMatches = false
    self.noMatchesTextY = -64
    self.reloadingBoardTextY = -64
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        self.availableMatches = self.board:checkAvailableMatches()
        if self.availableMatches == 0 then
            self.noMoreMatches = true
            Timer.tween(0.5, {
                [self] = {noMatchesTextY = VIRTUAL_HEIGHT / 2 - 8}
            })
            ...
            :finish(function()
                Timer.after(2, function()
                    ...
                    Timer.tween(0.5, {
                        [self] = {noMatchesTextY = VIRTUAL_HEIGHT + 30}
                    })
                    
                    :finish(function()
                        
                        Timer.tween(0.5, {
                            [self] = {reloadingBoardTextY = VIRTUAL_HEIGHT / 2 - 8}
                        })
                        ...
                        :finish(function()
                            Timer.after(2, function()
                                ...
                                Timer.tween(0.5, {
                                    [self] = {reloadingBoardTextY = VIRTUAL_HEIGHT + 30}
                                })
                                
                                :finish(function()
                                    while self.board:checkAvailableMatches() == 0 do
...
function PlayState:render()
    ...
    if self.noMoreMatches == true then
        love.graphics.setColor(95, 205, 228, 200)
        love.graphics.rectangle('fill', 0, self.noMatchesTextY - 8, VIRTUAL_WIDTH, 32)
        love.graphics.rectangle('fill', 0, self.reloadingBoardTextY - 8, VIRTUAL_WIDTH, 32)
        love.graphics.setColor(255, 255, 255, 255)
        love.graphics.setFont(gFonts['medium'])
        love.graphics.printf('No more matches!',
            0, self.noMatchesTextY, VIRTUAL_WIDTH, 'center')
        love.graphics.printf('Reloading Board!',
            0, self.reloadingBoardTextY, VIRTUAL_WIDTH, 'center')
    end
end