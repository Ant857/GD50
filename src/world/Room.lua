...
function Room:update(dt)
    ...
    for k, obj in ipairs(self.objects) do
        if obj.type == 'pot' and obj.projecting then
            local potSmash = false
            if obj.x <= MAP_RENDER_OFFSET_X + TILE_SIZE then
                potSmash = true
            elseif obj.x + obj.width >= VIRTUAL_WIDTH - TILE_SIZE * 2 then
                potSmash = true
            elseif obj.y <= MAP_RENDER_OFFSET_Y + TILE_SIZE - obj.height / 2 then
                potSmash = true
            else
                local bottomEdge = VIRTUAL_HEIGHT - (VIRTUAL_HEIGHT - MAP_HEIGHT * TILE_SIZE) + MAP_RENDER_OFFSET_Y - TILE_SIZE
                if obj.y + obj.height >= bottomEdge then
                    potSmash = true
                end
            end
    ...
    for i = #self.entities, 1, -1 do
        ...
        if entity.health <= 0 then
            ...
            if math.random(5)==1 and not entity.droppedItem then
        ...
        if not entity.dead then
            for k, obj in ipairs(self.objects) do
                if obj.type == 'pot' and entity:collides(obj) and obj.projecting then
                    entity:damage(1)
                    table.remove(self.objects, k)
                end
            end
