...
function Board:init(x, y, level)
    ...
    self.spawnableColors = self:getRandomColors()
    self.maxVariety = math.min(6, self.level)
...
function Board:initializeTiles()
    ...
    for tileY = 1, 8 do
        ...
        for tileX = 1, 8 do
            local variety = math.random(1, self.maxVariety)
            -- create a new tile at X,Y with a random color and variety
            table.insert(self.tiles[tileY], Tile(tileX, tileY, self.spawnableColors[math.random(8)], variety))
        end
    end
    ...
    for x = 1, 8 do
        for y = 8, 1, -1 do
            ...
            if not tile then
                ...
                local variety = math.random(1, self.maxVariety)
                local tile = Tile(x, y, self.spawnableColors[math.random(8)], variety)
                tile.y = -32
                self.tiles[y][x] = tile
...
function Board:checkAvailableMatches()
  ...
  local availableMatches = 0
  
    for y = 1, 8 do
        for x = 1, 7 do
            
            --swap tile and check if it is a match
            --if tile is a match then add one to availableMatches
            
            self:swapTiles(self.tiles[y][x], self.tiles[y][x + 1])
            
            local isMatch = self:calculateMatches()
            
            if isMatch then
                availableMatches = availableMatches + 1
            end
...
function Board:getRandomColors()
